...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    } else if (nHeight <= 35000) { // !!!!!!!!!!! TODO: 35k is testnet value, mainet is different, 100k ????
        // the initial split of 60% MN, 40% Stakers
        ret = blockValue  / 100 * 60;
    } else {
        // increasing the reward % as we are decreasing the block reward to make room for governance
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (!fLiteMode) {
        if (masternodeSync.RequestedMasternodeAssets > MASTERNODE_SYNC_LIST) {
            ...
            LogPrintf("%s : !!!! after obfuScationPool.NewBlock\n", __func__);
            masternodePayments.ProcessBlock(GetHeight() + 10);
            LogPrintf("%s : !!!! after masternodePayments.ProcessBlock\n", __func__);
            budget.NewBlock();
            LogPrintf("%s : !!!! after budget.NewBlock()\n", __func__);
        } else {
            LogPrintf("%s : !!!! masternodeSync.RequestedMasternodeAssets(%d) <= MASTERNODE_SYNC_LIST(%d) \n", __func__, masternodeSync.RequestedMasternodeAssets, MASTERNODE_SYNC_LIST);
        }
    } else {
